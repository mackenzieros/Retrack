...
class QuestionGenerator:
    ...
    def _determine_wh(self, clause, obj) -> str:
        ...
        for token in clause:
            # only subjects that are not the nominal subject and are not possessive nouns of the nominal subject are considered
            if token.ent_type > 0 and nlp.vocab[token.dep].text != 'nsubj' and nlp.vocab[token.head.dep].text != 'nsubj':
                ent = nlp.vocab[token.ent_type].text
                ents[ent].append(token.text)
            
            # if passive nominal subject, ask a how question
            if nlp.vocab[token.dep].text == 'nsubjpass':
                return 'How'

        if len(ents['PERSON']) > 0:
            return 'Who'
        elif len(ents['GPE']) > 0:
            return 'Where'
        elif len(ents['DATE']) > 0:
            return 'When'
        else:
            return 'What'
    ...
    def _map_syntax(self, start, end) -> dict or None:
        ...
        wh = self._determine_wh(clause, obj)
